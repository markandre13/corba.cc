...
class A_impl : public virtual A_skel {
    ...
    public:
        A_impl() : _id(++id) {}
        CORBA::async<void> speakA() override { println("speakA {}", _id); co_return; }
...
class D_impl : public virtual D_skel, public virtual B_impl /* , public virtual C_impl */ {
    public:
        CORBA::async<void> speakA() override { println("speakD {}", _id); co_return; }
        CORBA::async<void> sayD() override { println("sayD {}", _id); co_return; }
};
...
kaffeeklatsch_spec([] {
    ...
    describe("interface", [] {
        it("inheritance", [] {
            ...
            expect(a->_is_a("IDL:omg.org/CORBA/Object:1.0")).to.beTrue();
            expect(a->_is_a("IDL:A:1.0")).to.beTrue();
            expect(a->_is_a("IDL:C:1.0")).to.beFalse();

            expect(b->_is_a("IDL:omg.org/CORBA/Object:1.0")).to.beTrue();
            expect(b->_is_a("IDL:A:1.0")).to.beTrue();
            expect(b->_is_a("IDL:B:1.0")).to.beTrue();
        });
        ...
        it("send'n receive", [] {
            ...
            parallel(eptr, [&] -> async<> {
                ...
                auto frontend = make_shared<PeerSub_impl>();
